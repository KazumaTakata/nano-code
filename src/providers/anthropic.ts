...
// 第3章で実装したAnthropicプロバイダー（本リポジトリのtypes.tsに合わせて
// ToolCall.arguments / finishReason: 'tool_call' に適応済み）。
...
export function createAnthropic(config?: { apiKey?: string; maxRetries?: number }): Provider {
                ...
                const response = await client.messages.create(
                    {
                        model: modelId,
                        system,
                        messages: convertMessages(params.messages) as any,
