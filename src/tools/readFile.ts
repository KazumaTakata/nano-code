...
async function readFileExecute(args: { path: string }): Promise<string> {
  ...
  if (!realPath.startsWith(allowedPrefix) && realPath !== WORKSPACE_ROOT) {
    throw new Error(`アクセス拒否 ${realPath}はシンボリックリンク経由でワークスペース外を参照しています`)
  }
...
export const readFile = {
  ...
  description: 'ワークスペース内の指定されたパスのファイル内容を文字列として読み込む。ファイルが存在しない場合エラーを返す。100KBを超える巨大ファイルは読み込めない(コンテキストウィンドウ保護のため)。相対パスまたは絶対パスを指定できる。',
  parameters: {
    type: 'object',
    properties: {
      path: {
