...
async function runScenarios() {
    ...
    const netResult = await sandbox.run(
        'curl',
        ['-s', '--connect-timeout', '3', 'https://example.com'],
        {
            allowNetwork: false,
        },
    );
    ...
    const tmpResult = await sandbox.run('/bin/sh', [
        '-c',
        'echo "test" > /tmp/sandbox-test.txt && cat /tmp/sandbox-test.txt',
    ]);
    ...
    const results = [
        ...
        {
            name: 'Destructive command (rm /bin)',
            blocked: rmResult.exitCode !== 0,
            shouldBlock: true,
        },
        {
            name: 'Allowed operation (echo)',
            blocked: allowedResult.exitCode !== 0,
            shouldBlock: false,
        },
    ...
    for (const r of results) {
        ...
        const status = passed ? '✓' : '✗';
        console.log(
            `${status} ${r.name}: ${r.blocked ? 'Blocked' : 'Allowed'} (expected: ${r.shouldBlock ? 'Block' : 'Allow'})`,
        );
    }
