...
const messages: Message[] = [
    {
        role: 'user',
        content:
            'grepを使って0502docs.mdのコードを特定して、説明セクションの続きを0502docs.mdへ書き込んでください。コードの説明はフローチャートのように技術の詳細を省いた大まかな流れを記載してください。',
    },
];
...
while (true) {
    ...
    for (const tool of requestedTools ?? []) {
        ...
        // ツールは実行時にerrorをスローするため、catchしなければ思考ループが強制終了されてしまう。
        // そこで、エラーが発生した場合はcatchで受け取り、その内容を文字列としてLLMに返す。
        const content = await tool.execute(tool.arguments).catch((e) => `エラー: ${e.message}`);
